...
const CustomerProfileRoute = (req, res) => {
  ...
  if (req.method === "POST" && parsedUrl.pathname === "/api/login") {
    parseBody(req, async (body) => {
      ...
      const query2 = `
        select * from employee
        left join departments on employee.Employee_ID = departments.Department_Manager_ID
        where Email=? AND Password=?;
      `;

      try {
        const [results1] = await db.query(query1, [email, password]);
        const [results2] = await db.query(query2, [email, password]);

        if (results1.length > 0) {
          res.writeHead(200, { "Content-Type": "application/json" });
          res.end(JSON.stringify(results1[0]));
        } else if (results2.length > 0) {
          const employee = results2[0];
          res.writeHead(200, { "Content-Type": "application/json" });
        ...
        res.writeHead(500, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ message: "Error with login" }));
        throw err;
      }
    });
  } else {
